...
!function(root){
	...
	// Utility function to use native merthods on objects
	...
	// The underscore.belt configuration object
	var _config = {
		// The default selector engine
		// It is based on document.getElementById and QSA
		// If useLiveNodeList is true it also uses
		// document.getElementsByClass/Name/TagName
		defineEngine: function(s) {
			if (this.useLiveNodeList) {
				...
				if (fast) {
					...
					return document['getElement'+c](s.slice(1));
				} else {
	...
	// The _belt.config function
	// allows you to redefine the selector engine and activate/deativate
	// the use of live nodelists
